...
class CameraActivity : AppCompatActivity(), View.OnClickListener {
        ...
        imageCapture.takePicture(outputOption, ContextCompat.getMainExecutor(this),
            object : ImageCapture.OnImageSavedCallback{
                ...
                override fun onError(exception: ImageCaptureException) {
                    Toast.makeText(this@CameraActivity, getString(R.string.Failed_to_take_a_picture), Toast.LENGTH_SHORT).show()
                }
            })
    }
    ...
    private fun startCamera() {
        ...
        cameraProviderFuture.addListener({
            ...
            try {
                ...
                Toast.makeText(this, getString(R.string.Failed_to_open_camera),Toast.LENGTH_SHORT).show()
            }
    ...
    private fun permissionsNotGranted() {
        ...
        val alertDialog = AlertDialog.Builder(this).setTitle("Permissions required")
            with(alertDialog) {
                val remaining = 2
                setMessage("These permissions are required to use this app. \nPlease allow Camera permissions first," +
                        " ${remaining - userPreference.getGrant()} attempt remaining.")
                if (userPreference.getGrant() < 2){
                    setNegativeButton("don't allow") { _, _ -> finish() }
                    setPositiveButton("Grant") { _, _ ->
                        userPreference.setGrant()
